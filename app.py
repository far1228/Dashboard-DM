...
with st.sidebar:
    ...
    st.markdown("Jika tidak upload, data default akan digunakan.")
...
if uploaded_file:
    ...
    data_source = "⬆ Uploaded by User"
else:
    url = "https://raw.githubusercontent.com/far1228/Dashboard-DM/main/file.csv"
    df = pd.read_csv(url)
    data_source = "🌐 Loaded from GitHub"

# =================================================================================================
# MAIN CONTENT - DATASET OVERVIEW
# =================================================================================================
st.markdown('<div class="section-header"><h3>📊 Dataset Overview</h3></div>', unsafe_allow_html=True)
st.markdown(f"**Sumber Data:** {data_source}")
...
def run_complete_analysis_for_k(k, X, df_orig, preprocessor_obj, numerical_cols, categorical_cols, key_prefix=""):
    ...
    with st.expander("🔬 Analisis Dampak Outlier", expanded=False):
        if numerical_cols:  # Check if there are numerical columns
            selected_col = st.selectbox("Pilih Fitur:", numerical_cols, key=f"outlier_vis_{key_prefix}")
            st.plotly_chart(px.box(df_result, x='Outlier_Status', y=selected_col, color='Outlier_Status', title="Distribusi Fitur vs Status Outlier", color_discrete_map={'Outlier': '#EF553B', 'Non-Outlier': '#636EFA'}), use_container_width=True)
            
            if n_outliers > 0 and n_outliers < len(df_result):
                df_no_outliers = df_result[df_result['Outlier_Status'] == 'Non-Outlier'].copy()
                X_no_outliers = preprocessor_obj.transform(df_no_outliers[categorical_cols + numerical_cols])
                if len(df_no_outliers) > k:
    ...
    with tab1: 
        if numerical_cols:
            st.dataframe(df_result.groupby('Cluster')[numerical_cols].agg(['mean', 'std', 'count']).round(2), use_container_width=True)
        else:
    ...
    with tab2:
        ...
        with col1:
            if numerical_cols:
                ...
                st.info("Tidak ada kolom numerik tersedia.")
        with col2:
            if categorical_cols:
    ...
    with tab3:
        ...
        if numerical_cols:
            ...
            st.info("Tidak ada kolom numerik untuk menampilkan centroid.")
...
# =================================================================================================
    ...
    if n_clusters > 0:
        ...
        with tab1:
            ...
            if not valid_clusters_df.empty and numerical_cols:
                st.dataframe(valid_clusters_df.groupby('Cluster')[numerical_cols].agg(['mean', 'std', 'count']).round(2), use_container_width=True)
            else:
                st.info("Tidak ada klaster valid yang ditemukan untuk dianalisis atau tidak ada kolom numerik.")
        with tab2:
            valid_clusters_df = df_result[df_result['Cluster'] != -1]
        ...
        st.warning("⚠️ Tidak ada klaster yang ditemukan. Coba sesuaikan parameter Epsilon (eps) dan Minimum Samples.")
